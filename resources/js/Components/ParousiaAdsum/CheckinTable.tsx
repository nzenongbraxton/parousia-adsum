...
import { StatusBadge, type CheckinMethod } from "./StatusBadge";
...
export type LiveCheckin = {
  ...
  user_name: string;
  initials: string;
  /** AttendanceType value: 'telegram' | 'qr_kiosk' | 'sms' | 'whatsapp' */
  type: string;
  /** AttendanceStatus value: 'verified' | 'rejected' | 'pending' */
  status: string;
  time: string;
};

function toMethod(type: string, status: string): CheckinMethod {
  if (status === "rejected" || status === "pending") return "Flagged";
  switch (type) {
    case "telegram":  return "GPS-Verified";
    case "qr_kiosk":  return "QR-Kiosk";
    case "sms":       return "SMS-Entry";
    case "whatsapp":  return "IP-Verified";
    default:          return "Flagged";
  }
}

function toLocation(type: string): string {
  switch (type) {
    case "telegram":  return "Telegram GPS";
    case "qr_kiosk":  return "Kiosk Scan";
    case "sms":       return "SMS";
...
export function CheckinTable({ rows }: { rows?: LiveCheckin[] }) {
  // Fall back to static demo data when no live rows are passed.
  const isLive = rows !== undefined;
    ...
    <div className="glass-strong overflow-hidden rounded-2xl">
      <div className="flex items-center justify-between border-b px-4 py-3 md:px-5">
        <div>
          ...
          <p className="text-xs text-muted-foreground">Live feed • last 50 records</p>
        </div>
      ...
      {isLive ? (
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="mt-1.5 flex items-center justify-between gap-2">
                    <span className="inline-flex min-w-0 items-center gap-1 truncate text-[11px] text-muted-foreground">
                      ...
                      <span className="truncate">{toLocation(r.type)}</span>
                    </span>
                    <StatusBadge method={toMethod(r.type, r.status)} />
                  </div>
                </div>
              </li>
            ))}
                  ...
                  <TableRow key={r.id}>
                    <TableCell>
                      <div className="flex items-center gap-3">
                        ...
                        <p className="text-sm font-medium leading-tight">{r.user_name}</p>
                      </div>
                    </TableCell>
                    <TableCell className="text-sm text-muted-foreground">{r.time}</TableCell>
                    <TableCell className="text-sm text-muted-foreground">{toLocation(r.type)}</TableCell>
                    <TableCell>
                      <StatusBadge method={toMethod(r.type, r.status)} />
                    </TableCell>
                  </TableRow>
        ...
        <>
          {/* Mobile: stacked cards (static demo) */}
          <ul className="divide-y md:hidden">
